package com.example.d2m.screens.utils.calendar_utils.calendar
...
import com.example.d2m.screens.utils.calendar_utils.selection.CalendarDetailsLookup
import com.example.d2m.screens.utils.calendar_utils.selection.CalendarKeyProvider
        ...
        this.apply {
            ...
            if (dateList.isNullOrEmpty()) {
                dateList.apply {
                    ...
                    addAll(
                        DateUtils.getDates(
                            pastDaysCount, futureDaysCount, includeCurrentDate
                        )
                    )
                }
            }
            ...
            addOnScrollListener(object : RecyclerView.OnScrollListener() {
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    ...
                    val lastVisibleItem =
                        if (dx > 0) (layoutManager as LinearLayoutManager).findLastCompletelyVisibleItemPosition()
                        else (layoutManager as LinearLayoutManager).findFirstCompletelyVisibleItemPosition()

                    if (previousMonthNumber != DateUtils.getMonthNumber(dateList[lastVisibleItem]) || previousYear != DateUtils.getYear(
    ...
    private fun initSelection() {
        ...
        val selectionPredicate = object : SelectionTracker.SelectionPredicate<Long>() {
            ...
            override fun canSetStateAtPosition(position: Int, nextState: Boolean): Boolean =
                if (position != GHOST_ITEM_KEY) if (calendarSelectionManager.canBeItemSelected(
                        position, dateList[position]
                    )
                ) !(!nextState && !deselection) // if item is selected and deselection is
                else false // user cant select disabled items
                else true // select ghost item


            override fun canSetStateForKey(key: Long, nextState: Boolean): Boolean =
                if (key.toInt() != GHOST_ITEM_KEY) if (calendarSelectionManager.canBeItemSelected(
                        key.toInt(), dateList[key.toInt()]
                    )
                ) !(!nextState && !deselection) // if item is selected and deselection is
                else false // user cant select disabled items
                else true // select ghost item
        ...
        val selectionObserver = object : SelectionTracker.SelectionObserver<Long>() {
            override fun onItemStateChanged(key: Long, selected: Boolean) {
                ...
                if (key.toInt() != GHOST_ITEM_KEY && key.toInt() < dateList.size) calendarChangesObserver.whenSelectionChanged(
                    selected, key.toInt(), dateList[key.toInt()]
                )

                if (selectionTracker.selection.size() == 0) disableLongPress()
...
//        var bool =
//        GHOST_ITEM_KEY = position
        return selectionTracker.select(position.toLong())
    }
    ...
    fun getSelectedDates(): List<Date> {
        ...
        selectionTracker.selection.forEach {
            if (it.toInt() != GHOST_ITEM_KEY && it.toInt() < dateList.size) selectionList.add(
                dateList[it.toInt()]
            )
        }
    ...
    fun getSelectedIndexes(): List<Int> {
        ...
        selectionTracker.selection.forEach {
            if (it.toInt() != GHOST_ITEM_KEY && it.toInt() < dateList.size) selectionList.add(it.toInt())
        }
        return selectionList
    }
    ...
    fun setDates(newDateList: List<Date>) {
        if (::selectionTracker.isInitialized) clearSelection()
        dateList.clear()
        dateList.addAll(newDateList)
        adapter = SingleRowCalendarAdapter(
            newDateList, calendarViewManager
        )
        if (scrollPosition > dateList.size - 1) scrollPosition = dateList.size - 1
